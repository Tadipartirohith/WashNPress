...
export function registerRouteDocs(): void {
  ...
  doc("POST", "/v1/supervisor/slots", { summary: "Create a pickup slot", tags: ["Supervisor"], roles: ["supervisor"], body: obj({ societyId: str(), date: str("The day, as YYYY-MM-DD"), window: str(), startTime: str(), endTime: str(), capacityTotal: int("How many bookings the slot holds: a whole number from 2 to 30") }, ["societyId", "date", "window", "startTime", "endTime", "capacityTotal"]) });
  ...
  doc("POST", "/v1/supervisor/service-slots", { summary: "Create an additional-service slot", tags: ["Supervisor"], roles: ["supervisor"], body: obj({ societyId: str(), date: str("The day, as YYYY-MM-DD"), offeringId: str(), window: str(), capacity: int("How many bookings the slot holds: a whole number from 2 to 30") }, ["societyId", "date", "offeringId", "window", "capacity"]) });
  doc("PATCH", "/v1/supervisor/slots/:id", { summary: "Edit a slot", description: "Capacity cannot be lowered below what is already booked.", tags: ["Supervisor"], roles: ["supervisor"], params: { id: "Slot id" }, body: obj({ window: str(), startTime: str(), endTime: str(), capacityTotal: int("How many bookings the slot holds: a whole number from 2 to 30"), isActive: bool() }), responses: { "409": "Capacity is below the booked count" } });
  ...
  doc("GET", "/v1/supervisor/subscriptions", { summary: "Residents in this society and the plans they hold", description: "Read only. Plans are priced, sold and retired by the business, so a supervisor reads which resident is on which plan and does not create, edit or retire a plan — the routes that allowed it are gone rather than merely unlinked.", tags: ["Supervisor"], roles: ["supervisor"] });
  ...
  doc("POST", "/v1/admin/slots", { summary: "Create a pickup slot for any society", description: "Admin cover, so slot creation is never blocked by a supervisor being unavailable.", tags: ["Admin"], roles: ["admin"], body: obj({ societyId: str(), date: str("The day, as YYYY-MM-DD"), window: str(), startTime: str(), endTime: str(), capacityTotal: int("How many bookings the slot holds: a whole number from 2 to 30") }, ["societyId", "date", "window", "startTime", "endTime", "capacityTotal"]) });
  ...
  doc("POST", "/v1/admin/service-slots", { summary: "Create an additional-service slot (car wash, ironing, …)", description: "Unique per society + date + service + window. Only active additional-service offerings are eligible.", tags: ["Admin"], roles: ["admin"], body: obj({ societyId: str(), date: str("The day, as YYYY-MM-DD"), offeringId: str(), window: str(), capacity: int("How many bookings the slot holds: a whole number from 2 to 30") }, ["societyId", "date", "offeringId", "window", "capacity"]) });
  doc("PATCH", "/v1/admin/slots/:id", { summary: "Edit any slot", tags: ["Admin"], roles: ["admin"], params: { id: "Slot id" }, body: obj({ window: str(), startTime: str(), endTime: str(), capacityTotal: int("How many bookings the slot holds: a whole number from 2 to 30"), isActive: bool() }) });
  ...
  doc("DELETE", "/v1/admin/plans/:id", { summary: "Delete a plan", description: "Permanently removes a plan. Refused with 409 when the plan still has active subscriptions — deactivate it instead.", tags: ["Admin"], roles: ["admin"], params: { id: "Plan id" }, responses: { "409": "Plan still has active subscriptions" } });
  ...
  doc("POST", "/v1/admin/garment-categories", { summary: "Create a garment category", description: "A two-level garment category (I-71): a named group of garment items, each priced per piece. The flat per-garment prices are derived from these.", tags: ["Admin"], roles: ["admin"], body: obj({ name: str(), description: str(), status: str(), items: arr(obj({ name: str(), pricePaise: int() }, ["name", "pricePaise"])) }, ["name", "status", "items"]) });
  doc("PATCH", "/v1/admin/garment-categories/:id", { summary: "Edit a garment category", tags: ["Admin"], roles: ["admin"], params: { id: "Category id" }, body: obj({ name: str(), description: str(), status: str(), items: arr(obj({ name: str(), pricePaise: int() }, ["name", "pricePaise"])) }, ["name", "status", "items"]) });
  doc("DELETE", "/v1/admin/garment-categories/:id", { summary: "Delete a garment category", description: "Refused with 409 when a garment in the category is on an active order — deactivate it instead.", tags: ["Admin"], roles: ["admin"], params: { id: "Category id" }, responses: { "409": "A garment in the category is on an active order" } });
