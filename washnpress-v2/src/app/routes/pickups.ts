...
import { countField, requiredText } from "./form-fields";
...
const lineSchema = z.object({
  category: requiredText("Choose a garment category."),
  quantity: countField("Quantity"),
  serviceId: requiredText("Choose a service."),
...
const bookSchema = z.object({
  slotId: requiredText("Choose a pickup slot."),
...
const rescheduleSchema = z.object({ pickupId: requiredText("Which pickup?"), slotId: requiredText("Choose a pickup slot.") });
...
const cancelSchema = z.object({ pickupId: requiredText("Which pickup?") });
...
export function registerPickupRoutes(app: FastifyInstance, container: Container): void {
  ...
  app.get<{ Querystring: { slotId?: string; estimatedCount?: string; lines?: string } }>("/v1/pickups/preview", async (req, reply) => {
    ...
    if (!slotId) return reply.code(400).send({ error: "invalid_request", message: "Choose a pickup slot.", details: { formErrors: [], fieldErrors: { slotId: ["Choose a pickup slot."] } } });
    ...
    if (req.query.lines) {
      try {
        ...
        return reply.code(400).send({ error: "invalid_request", message: "The garment list could not be read.", details: { formErrors: [], fieldErrors: { lines: ["The garment list could not be read."] } } });
      }
    }
