...
// Dark mode is read live rather than baked in at module load: `themed()` builds one
// stylesheet per mode ahead of time, and the `theme`/`stateColor` proxies pick the
// right one by reading the active mode on every access, so a screen written against
// `theme.text.primary` years ago resolves against whichever mode is live right now.
//
// That makes these the load-bearing tests: if the proxy stops following the active
...
describe("reading the theme live", () => {
  ...
  it("moves the state colours as well, so a pill is not lit for the wrong ground", () => {
    // Set explicitly rather than relying on whatever the ambient default is: the
    // point under test is that switching modes changes the colour, not what the
    // starting mode happens to be.
    setColorScheme("light");
