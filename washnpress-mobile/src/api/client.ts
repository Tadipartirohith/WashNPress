...
export const api = {
  ...
  // The naming conventions on offer and what each one produces. The preview comes
  // from the backend because it is generated by the same functions that will
  // produce the real names.
  adminNaming: (params: Record<string, string | undefined>, token: string) =>
    request<NamingStyles>(`/v1/admin/naming${qs(params)}`, { token }),
